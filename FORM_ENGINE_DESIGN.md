...
6. **Consent questions are answered on the user's behalf, via a separate policy layer, not the interpreter** (founder decision, Jul 2026; implementation + correction, Aug 2026). Original framing said "checkboxes" — real corpus data shows `combobox` dominates every consent-shaped category, not checkboxes; corrected here. Built as `extension/consent_policy.js`, deliberately NOT inside `detect_structural_pattern()`/the interpreter and NOT inside `resolveValue()` — the interpreter only ever classifies (`consent_sms_communication`, etc.), a small dispatcher in `fillField()` routes to either `resolveConsentAnswer()` (policy) or `resolveValue()` (profile lookup). Full rationale, the category→default table, and why the split matters for what `replay.py` can and can't measure: `corpus_analysis/CONSENT_POLICY_SPEC.md`. Required consent (background check, privacy policy, GDPR notice) defaults accept; voluntary/optional consent (SMS opt-in, marketing opt-in) defaults decline. `consent_attestation_general` is excluded — corpus-verified contaminated with unrelated fact-based questions (see that spec doc).
